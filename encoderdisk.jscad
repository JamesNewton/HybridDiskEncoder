...
var assembly = [
    ...
    linear_extrude({height: thick},mask).translate([slotd/2,0,thick]).setColor([0,0,0]),
    linear_extrude({height: thick},disk).translate([0,0,thick*2]),
    linear_extrude({height: thick},arm).translate([0,0,thick*3])
        .setColor(1,0.5,0.3),
    linear_extrude({height: thick},washer).translate([0,0,thick*4]),
    ]
var cut = []
    cut.push(base)
    placeright(mask,cut)
